<?php
	...
	if (isset($_GET['vkey'])) {
		...
		if ($resultData->num_rows == 1) {
			...
			mysqli_stmt_bind_param($stmt, "s", $vkey);
			mysqli_stmt_execute($stmt);
			// NOTE: lil' verify message not working?
			header("location: ../index.php?page=pages/verify.php&verified=true");
			exit();
		} else {
			header("location: ../index.php?page=pages/verify.php&error=invalidoralreadyverified");
			exit();
		}
	}
?>
...
<section id="temp" class="content">
	Email verified, you can now <a href="index.php?page=pages/profile/account/login.php">log-in</a>! :D
